  ...
  function OrderService(
    ...
    this.initialize = function(
      ...
      return SalesforceService.loadOrderData(clientId, orderId).then(
        function(result) {
          ...
          if (orderData.order) {
            self.order.tipoCargaCliente__c = orderData.order.tipoCargaCliente__c;
            self.order.finDiscount = orderData.order.percDescFinanceiro__c;
            self.order.shipping = orderData.order.tipoFrete__c;
            self.paymentOptions = [...self.paymentOptions,...self.SpecialPaymentOptions];
            self.order.paymentOption = _.find(self.paymentOptions, {
    ...
    this.selectProduct = function(id) {
      ...
      if (product) {
        ...
        if (self.client.padraoPaletizacao__c === 'Cliente')
        {
            var paletizacao = self.Palletizations.filter(pallet => pallet.Produto__c === product.produtoId__c);

            if (paletizacao && paletizacao.length > 0)
            {
                product.Paletizacao = paletizacao[0].Altura__c * paletizacao[0].Base__c;
            }
        }
        else if (self.client.padraoPaletizacao__c === 'Carta Fabril')
        {
            product.Paletizacao = product.produtoId__r.Paletizacao__c;
        }

        if (product.Paletizacao && product.Paletizacao > 0)
        {
            product.QtdPaletizacao = product.saleQtd / product.Paletizacao;
        }
    ...
    this.quantidePallet = function() {
      return _.sumBy(self.selectedProducts, function(product) {
        return product.saleQtd > 0 ? (product.saleQtd / product.Paletizacao) : 0;
      });
    };
    ...
    this.canBeSaved = function(isDraft) {
      var haveQuantities,
        havePrices,
        attributesFilled,
        haveItens,
      ...
      havePaletizations = !isDraft ? _(productsErrors)
        .map(function(productError) {
          return _.isNil(productError.Produto__c);
        })
        .every() : true;

      if (!(haveQuantities && havePrices && havePaletizations)) {
    ...
    this.canSaveProducts = function(selectedProducts) {
      function canSaveProduct(product) {
        ...
        var naoContemPaletizacao = self.client.padraoPaletizacao__c === 'Cliente' && !self.Palletizations.some(pallet => pallet.Produto__c === product.produtoId__c);
        ...
        if (naoContemPaletizacao) {
          productError.Produto__c = 'produto não cadastrado na paletização';
        }
    ...
    this.buildHeader = function() {
      ...
      return {
        ...
        tipoCargaCliente__c : self.order.tipoCargaCliente__c
      };
    };
    ...
    self.saveOrder = function() {
      ...
      self.order.tipoCargaCliente__c = self.client.tipoDeCarga__c;
